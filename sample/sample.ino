...
int xwind,lwind;
int rpress,rtemp;
float temp_cor,pres_cor,wind_cor,bal_mill,dev_cor;
float rmill,elevation,pelevation,sel_range;
float net_c;  
double Range,net_range,elevation_cor;
uint8_t tt;
 char skey;
int tempvar;
  ...
  //int i=0,prev_i;
 // int x=0;
  int temp_x,press_x,wind_x,wind_l;
...
{ int ff;
  ...
  if(atype==0)//vog sheet1
  { sel_range = pgm_read_word_near(sheet1x+ v);
...
void compute()
{
  ...
  if(ltype==0)
  {
   pelevation=lwind; //headon
  }
  else if(ltype==1)
  {
   pelevation=-lwind; //headoff 6-12
  }
  else if(ltype==2)
  {
   pelevation=(lwind); 
   pelevation=pelevation*0.5; //1-7
  }
  else if(ltype==3)
  {
   pelevation=-(lwind); 
   pelevation=pelevation*0.5; //7-1
  }
 ...
 /* if(temp_x==15)
  ...
  {rtemp=rtemp*(-1);}
  */
  temp_cor=((temp_x-15.0)/10.0)*rtemp;
  if(temp_x>15)
  temp_cor=temp_cor*-1;
  Serial.print("temp_cor=");
  Serial.println(temp_cor,1);
 /* if(press_x==760)
  {rpress=0;}
  else if(press_x>760) 
  {rpress=rpress*1;}
  ...
  {rpress=rpress*(-1);}*/
  pres_cor=((press_x-760.0)/10.0)*rpress;
  if(press_x<760)
  pres_cor=pres_cor*-1; 
  Serial.print("pres_cor=");
  Serial.println(pres_cor,1);
  wind_cor=(pelevation/36.0)*wind_l;
  
  dev_cor=(xwind/36.0)*wind_x;
   Serial.print("dev_cor2=");
  Serial.println(dev_cor,1);
  if(stype==0)//3-9 xcrosswind
  {
    deviation=deviation+dev_cor;
  }
  else if(stype==1)
  {
    deviation=deviation-dev_cor;
  }
  Serial.print("wind_cor=");
  Serial.println(wind_cor,1);
 // net_c=pelevation+rtemp+rpress;
  net_c=temp_cor+wind_cor+pres_cor; 
  Serial.print("Correction=");
  Serial.println(net_c,1);
  net_range=Range+net_c;
  Serial.print("NRange=");
  Serial.println(net_range,0);
  Serial.print("Sel_range=");
  Serial.println(sel_range,0);
  elevation_cor=(net_range-sel_range)/(rmill);
  Serial.print("E Cor=");
  Serial.println(elevation_cor,0);
  elevation=elevation+elevation_cor;
   Serial.print("E final");
  Serial.println(elevation,0);
...
{ 
  ...
  show_msg(2000,"OK");
  wind_l=read_input_two("LWind(kmph)"); //wind
  show_msg(2000,"OK");
  wind_x=read_input_two("XWind(kmph)"); //wind
  show_msg(2000,"OK");
  ...
  do {
    ...
    u8g.print("(ELE)= ");u8g.print(elevation,0);
    u8g.setPrintPos(0, 60);
    u8g.print("(Range)= ");u8g.print(net_range);
  } while( u8g.nextPage() );
